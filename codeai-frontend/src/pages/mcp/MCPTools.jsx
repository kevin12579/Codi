        ...
        <div className="space-y-6">
            <div className="grid grid-cols-2 items-end mb-2">
                <div className="space-y-1">
                    <div className="flex items-center gap-2">
                        ...
                        <h1 className="text-3xl font-black text-[#0f172a] dark:text-white tracking-tight">MCP 도구 목록</h1>
                    </div>
                </div>
            ...
            {!loading && data && (
                <div className="bg-white dark:bg-slate-800 border border-[#e2e8f0] dark:border-slate-700 rounded-2xl p-6 shadow-sm">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {[
                            { label: '서버명', value: data.serverName },
                            { label: '버전', value: data.version },
                            { label: 'Transport', value: data.transport },
                        ].map(({ label, value }) => (
                            <div key={label}>
                                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1 block">{label}</label>
            ...
            {!loading && data?.tools && (
                <div className="space-y-3">
                    {data.tools.map((tool) => (
                        <div key={tool.name} className="bg-white dark:bg-slate-800 border border-[#e2e8f0] dark:border-slate-700 rounded-2xl p-5 shadow-sm space-y-3">
                            <div className="flex items-center gap-2">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <div className="flex flex-wrap gap-1.5">
                                        {Object.entries(tool.parameters).map(([key, val]) => (
                                            <div key={key} className="flex items-center gap-1.5 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg px-2.5 py-1.5">
                                                <span className="text-[10px] font-black text-slate-700 dark:text-slate-300 font-mono">{key}</span>
