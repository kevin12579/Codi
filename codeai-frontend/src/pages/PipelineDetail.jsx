...
import { ArrowLeft, GitFork, User, Calendar, Clock, CheckCircle2, XCircle, Bell, Copy, Check, Terminal } from 'lucide-react'
  ...
  const statusClass = (status) => {
    if (status === 'Success') return 'bg-[#e8f7ee] text-[#22c55e]'
    if (status === 'Failed') return 'bg-[#fee2e2] text-[#ef4444]'
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#e2e8f0] pb-6">
        <div className="space-y-2">
          ...
          <p className="text-sm font-semibold text-slate-400">{pipeline.repo}</p>
        </div>
        <div className="flex flex-wrap gap-x-6 gap-y-2 bg-white px-5 py-3 border border-[#e2e8f0] rounded-2xl text-xs font-medium text-slate-600">
          <div className="flex items-center space-x-1.5">
            ...
            <span className="font-mono bg-slate-100 px-1.5 py-0.5 rounded">{pipeline.branch}</span>
          </div>
          <div className="flex items-center space-x-1.5">
          ...
          <div className="flex items-center space-x-1.5">
            ...
            <span>2026년 6월 10일</span>
          </div>
          <div className="flex items-center space-x-1.5 font-bold text-slate-700">
            <Clock size={13} className="text-[#0066ff]" />
            <span>소요 시간: {pipeline.timing}</span>
          </div>
        </div>
      </div>
        ...
        <section className="lg:col-span-5 space-y-4">
          <div className="bg-white border border-[#e2e8f0] rounded-2xl p-6 shadow-sm">
            ...
            <div className="relative pl-6 space-y-8">
              ...
              {pipeline.timeline && pipeline.timeline.map((step, idx) => (
                <div key={idx} className="relative flex flex-col space-y-1">
                  <div className="absolute -left-[25px] top-0.5 w-[14px] h-[14px] rounded-full flex items-center justify-center bg-white">
                    {step.status === 'Success' ? (
                      ...
                      <div className="w-[14px] h-[14px] rounded-full bg-slate-100 border border-slate-300" />
                    )}
                  </div>
                  ...
                  <p className="text-xs text-slate-500 whitespace-pre-wrap leading-relaxed">{step.logs}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="lg:col-span-7 space-y-4">
          ...
          {activeTab === 'review' && (
            <div className="space-y-4">
              {pipeline.aiComments && pipeline.aiComments.length > 0 ? (
                pipeline.aiComments.map((comment) => (
          ...
          {activeTab === 'tests' && (
            <div className="space-y-6">
              {pipeline.testResults?.suites?.length > 0 ? (
                pipeline.testResults.suites.map((suite, idx) => (
                  <div key={idx} className="bg-white border border-[#e2e8f0] rounded-2xl p-6 shadow-sm space-y-5">
                    <div className="flex items-center justify-between border-b border-[#f1f5f9] pb-3">
                      <h4 className="text-sm font-black text-slate-900">{suite.name}</h4>
                      <span className="text-xs font-mono font-medium text-slate-400">소요: 45초</span>
                    </div>
                    <div className="grid grid-cols-3 gap-4 text-center">
                      <div className="bg-slate-50 p-4 rounded-xl space-y-1">
                        ...
                        <h5 className="text-2xl font-black text-slate-900 font-mono mt-1">{suite.passed}</h5>
                      </div>
                      <div className="bg-slate-50 p-4 rounded-xl space-y-1">
                        <span className="text-[10px] font-bold text-slate-400 tracking-wider flex items-center justify-center space-x-1">
                          <span className="w-1.5 h-1.5 rounded-full bg-[#ef4444]" />
                ...
                <div className="bg-white border border-[#e2e8f0] rounded-2xl p-12 text-center text-slate-500 space-y-2">
                  <h4 className="font-bold text-slate-900 text-sm">테스트 데이터 없음</h4>
                </div>
              )}
              <div className="bg-white border border-[#e2e8f0] rounded-2xl p-6 shadow-sm flex items-center justify-between">
                ...
                <div className="flex items-center space-x-3 bg-[#e6f0ff]/50 px-4 py-2.5 rounded-2xl">
                  <span className="font-mono text-lg font-black text-[#0066ff]">{pipeline.testResults.coverage}%</span>
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === 'logs' && (
            <div className="space-y-4">
              {pipeline.notifications && pipeline.notifications.map((notif, idx) => {
                const isFail = notif.event.includes('실패')
                const isBuild = notif.event.includes('빌드')
                return (
                  <div key={idx} className="bg-white border border-[#e2e8f0] rounded-2xl p-5 shadow-sm flex items-start justify-between gap-4">
                    <div className="flex items-start space-x-4">
                      ...
                      <div className="space-y-0.5">
                        <h4 className="text-sm font-bold text-slate-900">{notif.event}</h4>
                        <p className="text-xs text-slate-500">{notif.details}</p>
                      </div>
                    </div>
